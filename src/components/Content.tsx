...
import { ReadContracts } from '../backend/ReadContracts';
import { Utilization } from '../backend/ReadContract';
import '../static/css/content_card.css';
import '../static/css/content.css';

export function Content() {
  const nftStatsRef = useRef(null);
  const [nftStatsVisible, setNftStatsVisible] = useState<'hidden' | 'visible'>(
    'hidden'
  );
  const { data, isLoading, isSuccess } = ReadContracts();

  const nftStatsVariants: {
    [key in 'hidden' | 'visible']: { opacity: number; y: number };
  } = {
  ...
  function borrowPopup() {
    try {
      let k = document.getElementsByClassName('borrow_popup')[0] as HTMLElement;
  ...
  return (
    <div className="content_holder">
      <div className="contn">
        ...
        <div>
          Unlock the Value of Your NFTs with Seamless Lending and Borrowing.
        </div>
        <div className="bttn_holder">
          <input
            className="bttn_holder_1"
            type="button"
            value={'Lend'}
            onClick={lendPopup}
          />
          <input
            className="bttn_holder_2"
            type="button"
            value={'Borrow'}
            onClick={borrowPopup}
          />
      ...
      >
        <div className="left_stats">
          <div className="left_stats_one">Protocol's</div>
          <div className="left_stats_two">Statistics</div>
          <br />
          <div className="left_stats_three">
            To ensure you stay informed and don't miss any <br /> important
            updates.
          </div>
        </div>
          ...
          <div className="right_stats_card">
            ...
            >
              ...
              <div>
                ...
                <br />
                <span>
                  {data.totalBorrow} ETH{' '}
                  <small>
                    {' '}
                    <br />({(data.totalBorrow * data.ethTousd).toFixed(2)} USD)
                  </small>
                </span>
              </div>
            </div>
      ...
      <div className="card_list">
        <div className="heading_cards">NFTs Auction</div>
