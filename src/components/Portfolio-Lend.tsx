  ...
  return (
    <div className="lend_history_card">
      ...
      <div className="lend_history_content1">
        <div>
          ...
          <span>
            {deposit.Amount} ETH{' '}
            <small>
              ({(deposit.Amount * deposit.EthToUsd).toFixed(2)} USD)
            </small>
          </span>
        </div>
        <div>
          Interest <br />
          <span>
            {deposit.Interest} ETH{' '}
            <small>
              ({(deposit.Interest * deposit.EthToUsd).toFixed(2)} USD)
            </small>
          </span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <button
          className="portfolio_lend_bttn"
          onClick={() => {
            userWithdrawFromPool();
          }}
        >
          Withdraw
        </button>
